...
local createButton = function(cmd, icon, name, labelConnected, labelDisconnected, signal)
	...
	awesome.connect_signal('signal::' .. signal, function(status)
		if status then
			...
			widget:get_children_by_id("label")[1].markup = helpers.colorize_text(labelDisconnected,
...
local widget       = wibox.widget {
	{
		createButton("~/.config/awesome/scripts/wifi --toggle", "󰤨 ", "<b>Network</b>", "Connected", "Disconnected",
			"network"),
		createButton("~/.config/awesome/scripts/bluetooth --toggle", "󰂯 ", "<b>Bluetooth</b>", "Connected", "Disconnected",
			"bluetooth"),
		spacing = 20,
		layout = wibox.layout.flex.horizontal
	},
	{
		createButton("~/.config/awesome/scripts/airplane --toggle", "󰀝 ", "<b>Airplane Mode</b>", "Now In Flight Mode",
			"Turned Off", "airplane"),
		createButton('awesome-client \'naughty = require("naughty") naughty.toggle()\'', "󰍶 ",
			"<b>Do Not Disturb</b>",
			"Turned On",
			"Turned Off",
			"dnd"),
