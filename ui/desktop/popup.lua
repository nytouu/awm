	...
	keypressed_callback = function(self, mod, key, command)
		...
		elseif key == "BackSpace" then
			popup.prompt:get_children_by_id("txt")[1].markup =
				popup.prompt:get_children_by_id("txt")[1].markup:sub(1, -2)
...
popup.start = function(fn)
	if fn == "create" then
		...
		popup.func = function(name)
			awful.spawn.with_shell("touch ~/docs/desktop/'" .. name .. "'")
		end
	elseif fn == "folder" then
		popup.label = "Create A Folder"
		popup.name.markup = popup.label
		popup.func = function(name)
			awful.spawn.with_shell("mkdir ~/docs/desktop/'" .. name .. "'")
		end
	else
		...
		popup.func = function(name)
			...
			awful.spawn.with_shell("mv ~/docs/desktop/'" .. n .. "' ~/docs/desktop/'" .. name .. "'")
		end
	end
end
...
awesome.connect_signal("toggle::popup", function(fn)
	...
	awful.placement.centered(popup.widget, { honor_workarea = true })
end)
