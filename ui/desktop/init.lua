...
awful.placement.maximize(desktopdisplay, { margins = { top = beautiful.bar_height } })
...
local function gen()
	...
	for entry in io.popen([[ls ~/docs/desktop | sed '']]):lines() do
		...
		elseif os.execute("wc -c < ~/docs/desktop/'" .. entry .. "'") then
			...
			exec = "st -e nvim" .. " ~/docs/desktop/'" .. entry .. "'"
	...
	local iconmenu = awful.menu({
		items = {
			{ "Open", exec },
			{
				"Delete",
				function()
					awful.spawn.with_shell("trash " .. "~/docs/desktop/'" .. label .. "'")
				end,
			},
		},
	})
	...
	widget:connect_signal("button::press", function(_, _, _, button)
		if not mousegrabber.isrunning() then
			...
			mousegrabber.run(function(mouse)
				if
					(math.abs(mouse.x - startpos.x) > 10 or math.abs(mouse.y - startpos.y) > 10)
					and mouse.buttons[1]
				then
					grid:remove(widget)
				...
				if not mouse.buttons[1] then
					if button == 1 then
						if heldwidget.visible then
							...
							local newrow, newcol = gridindexat(mouse.y, mouse.x)
							if not grid:get_widgets_at(newrow, newcol) then
								...
								if elem.widget.exec:find("thunar ~/docs/desktop") then
									if exec ~= "Trash" then
										os.execute(
											"mv ~/docs/desktop/'"
												.. label
								...
								elseif elem.widget.exec:find("thunar trash:/") then
									if exec ~= "Trash" then
										os.execute("trash ~/docs/desktop/'" .. label .. "'")
									end
								else
...
local function load()
	...
	for _, entry in ipairs(layout) do
		grid:add_widget_at(
			createicon(entry.widget.icon, entry.widget.label, entry.widget.exec, entry.type),
			entry.row,
			entry.col
		)
	end
...
local subscribe = [[
   bash -c "
   while (inotifywait -m -e close_write -e delete -e create -e moved_from $HOME/docs/desktop/ -q) do echo; done
"]]

awful.spawn.easy_async_with_shell(
	"ps x | grep \"inotifywait -m -e close_write -e delete -e create -e moved_from $HOME/docs/desktop/ -q\" | awk '{print $1}' | xargs kill",
	function()
